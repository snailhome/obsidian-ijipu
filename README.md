...
- 语法解析失败会显示错误信息，不阻断正文。
- **显示模式**：整页 / 满宽 / 谱面，用工具栏下拉切换（默认**谱面**，自动消除页边距、最大化有效观看面积）。
- **排版**（田字格图标，与 iJipu 应用顶栏一致）：打开对话框改**这一份谱**的页面/字体/行距/渲染设置 →
  「**保存到谱面**」写回源码的 `# jps-config` 行（优先级最高，与 iJipu「保存设置」等价），
  或「保存为插件默认」写入全局默认。工具栏会显示来源徽标（绿=谱面自带设置 / 灰=frontmatter 覆盖）。
